...
async def require_key(x_api_key: Optional[str], session=None) -> bool:
    ...
    key_hash = service_key_hash(x_api_key)
    service = (await session.execute(select(Service).where(Service.api_key_hash == key_hash))).scalars().first()
...
async def get_incidents(limit: int = 50, status_filter: Optional[str] = None, service_id: Optional[str] = None, x_api_key: Optional[str] = Header(None)):
    async with AsyncSessionLocal() as session:
        await require_key(x_api_key, session)
        limit = max(1, min(limit, 200)); query = select(IncidentReport).order_by(desc(IncidentReport.created_at)).limit(limit)
...
async def get_incident(incident_id: str, x_api_key: Optional[str] = Header(None)):
    async with AsyncSessionLocal() as session:
        await require_key(x_api_key, session)
        try: parsed_id = uuid.UUID(incident_id)
...
async def update_incident_status(incident_id: str, body: StatusUpdate, x_api_key: Optional[str] = Header(None)):
    ...
    async with AsyncSessionLocal() as session:
        ...
        incident = await session.get(IncidentReport, parsed_id)
        if not incident: raise HTTPException(status_code=404, detail="Incident not found")
        incident.status = body.status; incident.resolved_at = datetime.now(timezone.utc) if body.status == "RESOLVED" else None
...
async def register_service(payload: ServiceCreate, x_api_key: Optional[str] = Header(None)):
    ...
    async with AsyncSessionLocal() as session:
        await require_key(x_api_key, session)
        if await session.get(Service, payload.id.strip()): raise HTTPException(status_code=409, detail="Service ID already exists")
        ...
        session.add(service); await session.commit(); await session.refresh(service)
...
def incident_to_dict(incident: IncidentReport) -> dict:
    return {"id": str(incident.id), "service_id": incident.service_id, "anomaly_score": incident.anomaly_score or 0.0, "status": incident.status, "error_type": incident.error_type, "raw_stack_trace": incident.stack_trace or "", "ai_root_cause": incident.ai_root_cause, "ai_suggested_patch": incident.ai_suggested_patch, "created_at": incident.created_at.isoformat() if incident.created_at else None, "resolved_at": incident.resolved_at.isoformat() if incident.resolved_at else None}
